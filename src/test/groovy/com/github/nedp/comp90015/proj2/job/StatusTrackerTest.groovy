...
class StatusTrackerTest extends Specification {
    ...
    def "new -> WAITING"() {
        expect: statusTracker.current() == WAITING
    }

    def "start -> RUNNING"() {
        setup: statusTracker.start()
        expect: statusTracker.current() == RUNNING
    }

    def "finish(_) -> assertion error"() {
        when: statusTracker.finish(ok)
    ...
    def "start->finish(ok) -> depends on ok"() {
        ...
        expect: statusTracker.current() == want

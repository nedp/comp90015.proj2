...
class MasterCLITest extends Specification {
    ...
    def "should pass command names to the factory producer"() {
        given: aStandardTestObject()
        and: interaction { inputIsJust input }

        when: underTest.run()
        then: 1 * factoryProducer.fromName(name) >> Optional.empty()
    ...
    def "should pass param scanners to command factories"() {
        given: aStandardTestObject()
        and: def factory = Mock factoryClass
        and: interaction {
            inputIsJust "asdasd"
        ...
        when: underTest.run()
        then: factory.fromParams(_ as Scanner) >> command
    ...
    def "should report bad commands"() {
        ...
        when: underTest.run()
        then: interaction { anythingAllowedWith out }
    }
    ...
    def "should run commands on the job manager, worker manager, and output"() {
        ...
        when: underTest.run()
        then: interaction { 1 * command.runOn(jobs, workers, out) }
